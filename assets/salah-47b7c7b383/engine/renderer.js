...
export class SalahRenderer {
  constructor(canvas) {
    ...
    // lets load() ask "are the programs ready?" WITHOUT waiting for the answer (Chromium; absent in Firefox - see load())
    this.parallelExt = gl.getExtension('KHR_parallel_shader_compile')
    this.stats = { textureBytes: 0, gpuMs: null, cpuMs: 0, frames: 0, birds: 0, compile: null }
    this.units = {}
  ...
  /** Starts compiling the two programs and returns at once. They take seconds on a first visit, and ASKING for their status
      makes the main thread wait for the GPU process - the page froze for ~2 s. With KHR_parallel_shader_compile they are
      started here, compile in the background, and load() asks about them only when the browser says they are complete.
      Without the extension (Firefox) this does nothing: there the work only happens when the status is asked, so load()
      compiles and asks at its end, as it always did. load() calls prepare() itself; a caller MAY call it earlier (it needs
      no asset) to have the programs ready by the time the scene is wanted. Calling it again changes nothing. */
  prepare() {
    if (this.pending !== undefined) return
    this.pending = this.parallelExt && !this.gl.isContextLost() ? [this.startProgram(VERTEX, REFLECT_FRAGMENT), this.startProgram(VERTEX, SCENE_FRAGMENT)] : null
    if (this.pending) this.gl.flush() // or the commands wait in the buffer and nothing compiles until the first question
  }

  /** Loads every GPU resource. Resolves WITHOUT programs if the WebGL context is lost meanwhile: such a renderer must not
      be used (callers check `gl.isContextLost()`); a fresh SalahRenderer is built after the restoration. */
  async load(base, onProgress = () => {}) {
    const gl = this.gl
    this.prepare() // the programs compile while the assets download and decode
    ...
    if (pending && !(await this.whenCompiled(pending))) return // context lost while waiting: nothing to finish, nothing to draw with
    try {
      this.progReflect = pending ? this.finishProgram(pending[0]) : this.program(VERTEX, REFLECT_FRAGMENT)
      this.progScene = pending ? this.finishProgram(pending[1]) : this.program(VERTEX, SCENE_FRAGMENT)
    } catch (error) {
      if (gl.isContextLost()) return // a dead context answers every question with null: that is a loss, not a shader error
      throw error
    }
    if (!pending) this.stats.compile = { parallel: false }
  ...
  program(vsSrc, fsSrc) {
    return this.finishProgram(this.startProgram(vsSrc, fsSrc))
  }

  /** Compile and link, asking NOTHING: every status question blocks until the GPU process has finished the work. */
  startProgram(vsSrc, fsSrc) {
    const gl = this.gl
    const mk = (type, src) => {
    ...
    const shaders = [mk(gl.VERTEX_SHADER, vsSrc), mk(gl.FRAGMENT_SHADER, fsSrc)]
  ...
  /** Resolves true once the browser reports every pending program complete - asked once per timer tick, never in a loop,
      so the page stays responsive (a timer, not an animation frame: it must also end in a background tab). Resolves false
      if the context is lost. After `limitMs` it stops waiting and lets finishProgram() block, exactly as before the
      extension was used: slower, never stuck. */
  whenCompiled(pending, limitMs = 20000) {
    const gl = this.gl, done = this.parallelExt.COMPLETION_STATUS_KHR, t0 = performance.now()
    return new Promise((resolve) => {
      const ask = () => {
        if (gl.isContextLost()) return resolve(false)
        const complete = pending.every(({ p }) => gl.getProgramParameter(p, done)), waited = performance.now() - t0
        if (!complete && waited < limitMs) return setTimeout(ask, 16)
        this.stats.compile = { parallel: true, waitedMs: Math.round(waited), gaveUp: !complete }
        resolve(true)
      }
      ask()
    })
  ...
  /** The real verdict - completion is not success: compile and link status, then the uniform table. */
  finishProgram({ p, shaders }) {
    const gl = this.gl
    for (const s of shaders) if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) throw new Error('shader: ' + gl.getShaderInfoLog(s))
